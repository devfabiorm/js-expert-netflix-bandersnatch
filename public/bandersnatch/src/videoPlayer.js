class VideoMediaPlayer {
  ...
  initializeCodec() {
    ...
    if(!mediaSourceSupported) {
      alert('Seu brower ou sistema não tem suporte a MSE!');
      return;
    }

    const codecSupported = MediaSource.isTypeSupported(this.manifestJSON.codec);
    if(!codecSupported) {
      alert(`Seu browser não suporta do codec: ${this.manifestJSON.codec}`);
      return;
    }
  ...
  waitForQuestions() {
    ...
    if(!option) return;
    //Evita que o modal apareça 2x no mesmo segundo
  ...
  async nextChunk(data) {
    ...
    //deixa o restante do vídeo rodar, baseado no tempo corrente
